    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          client: reference.client?._id || "",
          technologies:
            reference.technologies?.map((tech: any) => tech._id) || [],
          country: reference.country?._id || "",
          status: reference.status || "Actif",
          startDate: reference.startDate
            ? reference.startDate.split("T")[0]
            : "",
          endDate: reference.endDate ? reference.endDate.split("T")[0] : "",
          projectUrl: reference.projectUrl || "",
          githubUrl: reference.githubUrl || "",
          features: reference.features?.length > 0 ? reference.features : [""],
          challenges: reference.challenges || "",
          results: reference.results || "",
          testimonial: {
            content: reference.testimonial?.content || "",
            author: reference.testimonial?.author || "",
            position: reference.testimonial?.position || "",
          },
        });

        // Set existing files
        if (reference.files) {
          setUploadedFiles(reference.files);
        }
      } else {
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600">
              Mettez à jour les informations de votre projet IBTIKAR
            </p>
          </div>
        </div>
